...
if (typeof jQuery === 'undefined') throw new Error("Bootstrap's JavaScript requires jQuery"); +function(a) { const b = a.fn.jquery.split(' ')[0].split('.'); if (b[0] < 2 && b[1] < 9 || b[0] == 1 && b[1] == 9 && b[2] < 1) throw new Error("Bootstrap's JavaScript requires jQuery version 1.9.1 or higher"); }(jQuery), +function(a) {
  ...
  'use strict'; function b(b) {
    ...
    let e = this.$element.find('.item.active'),
      ...
      h = b == 'next' ? 'left' : 'right',
      i = b == 'next' ? 'first' : 'last',
      j = this; if (!f.length) { if (!this.options.wrap) return; f = this.$element.find('.item')[i](); } if (f.hasClass('active')) return this.sliding = !1; const k = f[0],
  ...
  'use strict'; function b(b) {
    ...
    if (/(38|40|27|32)/.test(b.which)) {
      const d = a(this); if (b.preventDefault(), b.stopPropagation(), !d.is('.disabled, :disabled')) {
        const e = c(d),
          g = e.hasClass('open'); if (!g && b.which != 27 || g && b.which == 27) return b.which == 27 && e.find(f).trigger('focus'), d.trigger('click'); const h = ' li:not(.divider):visible a',
  ...
  'use strict'; function b(b) {
    ...
    const b = a.Event('show.bs.' + this.type); if (this.hasContent() && this.enabled) {
      this.$element.trigger(b); const d = a.contains(this.$element[0].ownerDocument.documentElement, this.$element[0]); if (b.isDefaultPrevented() || !d) return; const e = this,
        ...
        .data('bs.' + this.type, this), this.options.container ? f.appendTo(this.options.container) : f.insertAfter(this.$element); const k = this.getPosition(),
        l = f[0].offsetWidth,
    ...
    let d = this.tip(),
      ...
      h = parseInt(d.css('margin-left'), 10); isNaN(g) && (g = 0), isNaN(h) && (h = 0), b.top = b.top + g, b.left = b.left + h, a.offset.setOffset(d[0], a.extend({ using(a) { d.css({ top: Math.round(a.top), left: Math.round(a.left) }); } }, b), 0), d.addClass('in'); const i = d[0].offsetWidth,
      j = d[0].offsetHeight; c == 'top' && j != f && (b.top = b.top + f - j); const k = this.getViewportAdjustedDelta(c, b, i, j); k.left ? b.left += k.left : b.top += k.top; const l = /top|bottom/.test(c),
    ...
    b = b || this.$element; let c = b[0],
      d = c.tagName == 'BODY',
      e = c.getBoundingClientRect(); e.width == null && (e = a.extend({}, e, { width: e.right - e.left, height: e.bottom - e.top })); const f = d ? { top: 0, left: 0 } : b.offset(),
  ...
  'use strict'; function b(b) {
    ...
    const e = this.$target.scrollTop(),
      ...
      g = this.$target.height(); if (c != null && this.affixed == 'top') return c > e ? 'top' : !1; if (this.affixed == 'bottom') return c != null ? e + this.unpin <= f.top ? !1 : 'bottom' : a - d >= e + g ? !1 : 'bottom'; const h = this.affixed == null,
      i = h ? e : f.top,
    ...
    if (this.$element.is(':visible')) {
      let b = this.$element.height(),
        ...
        this.unpin != null && this.$element.css('top', ''); const i = 'affix' + (h ? '-' + h : ''),
